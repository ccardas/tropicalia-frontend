...
const TrainModel = () => {
  ...
  const doTrain = async () => {
    await fetch(
      `http://0.0.0.0:8001/api/v1/algorithm/train?algorithm=${model}&crop_type=${cropType}`,
      {
        method: "GET",
        headers: { 
  ...
  return (
    <>
      <Space direction="vertical" size={10}>
        ...
        <Paragraph>
          Por favor, seleccione un modelo y un conjunto de datos para entrenar el modelo.
        </Paragraph>
        <Paragraph>
          <Alert
        ...
        <Button type="primary" onClick={doTrain} block disabled={!model || !cropType}>
          Entrenar
        </Button>
      </Space>
    </>
